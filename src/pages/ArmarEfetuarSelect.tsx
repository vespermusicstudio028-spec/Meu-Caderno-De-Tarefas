...
export function ArmarEfetuarSelectPage() {
  const navigate = useNavigate();
  const { armarEfetuarProgress, studyDaysCount } = useAppContext();
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-2xl font-black text-[#2D3436] uppercase tracking-tight flex items-center gap-2" style={{ fontFamily: '"Arial Black", sans-serif' }}>
          <span>🧮</span> ARMAR E EFETUAR
        </h2>
      </div>

      <div className="text-center py-2">
        <p className="text-lg text-[#636E72] font-bold">Escolha qual operação você quer treinar:</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {ops.map((op) => {
          const currentDay = (armarEfetuarProgress && armarEfetuarProgress[op.id]) || 1;
          const isTest = currentDay > studyDaysCount;
          return (
            <button
              key={op.id}
              onClick={() => navigate(`/tarefa/armar-efetuar/${op.id}`)}
